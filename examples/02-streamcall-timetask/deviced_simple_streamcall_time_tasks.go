...
func main() {
	...
	for i = 0; i < 2; i++ {
		...
		payload := &air_switch_pb.MqttDeviceRequest{
			Payload: &air_switch_pb.MqttDeviceRequest_TimeTaskReq{
				TimeTaskReq: &air_switch_pb.TimeTaskReq{},
			},
		}
		payload.GetTimeTaskReq().TimeTasks = append(payload.GetTimeTaskReq().TimeTasks,
			&air_switch_pb.TimeTask{
				Time: "0 56 13 * * *",
			})
		payload.GetTimeTaskReq().TimeTasks[0].Tasks = append(payload.GetTimeTaskReq().TimeTasks[0].Tasks,
		...
		payload.GetTimeTaskReq().TimeTasks[1].Tasks = append(payload.GetTimeTaskReq().TimeTasks[1].Tasks,
			&air_switch_pb.MqttDeviceRequest{
				SessionId: 99,
				Payload: &air_switch_pb.MqttDeviceRequest_Cmd_06Req{
					Cmd_06Req: &air_switch_pb.CmdSubReq{

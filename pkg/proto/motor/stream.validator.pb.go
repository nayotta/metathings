...
func (this *StreamSetSpeedRequest) Validate() error {
	...
	if nil == this.Speed {
		return github_com_mwitkow_go_proto_validators.FieldError("Speed", fmt.Errorf("message must exist"))
	}
	if this.Speed != nil {
...
func (this *StreamRequest) Validate() error {
	...
	if oneOfNester, ok := this.GetPayload().(*StreamRequest_SetDirection); ok {
		if oneOfNester.SetDirection != nil {
			if err := github_com_mwitkow_go_proto_validators.CallValidatorIfExists(oneOfNester.SetDirection); err != nil {
				return github_com_mwitkow_go_proto_validators.FieldError("SetDirection", err)
			}
		}
	}
